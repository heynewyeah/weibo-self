#!/bin/sh
# =============================================================
# query_image.sh — 查询图片类博文并保存到 HDFS
# =============================================================
# 功能：
#   从 ods_ad_sfst_media_info 关联 dm_wb_ad_sfst_multi_day，
#   提取汽车行业广告博文中 media_type=1（图片）的素材记录，
#   同时关联 ods_tblog_content 获取博文文字内容，
#   结果写入 HDFS 个人目录，供后续图文多模态分类任务使用。
#
# 数据来源：
#   主表: dplus_dm.dm_wb_ad_sfst_multi_day  （广告投放数据，筛选汽车行业）
#   关联: ods_tblog_content                  （博文原始文字内容）
#   关联: ods_ad_sfst_media_info             （素材详情，media_type=1 图片）
#   过滤: market_industry_name='汽车', bid_type=4, media_type=1
#
# 关联说明：
#   dm表.cust_uid = media表.customer_id（广告主维度关联）
#   customer_info 格式：JSON数组，元素为图片 pid，如 ["pid1","pid2"]
#
# 输出路径：
#   HDFS: /dw_ext/ad/person/xuanyu11/intent_behavior/data/
#         image_weibo_ad_<START_DAY>_<END_DAY>/
#   格式: TSV，字段分隔符 \t
#   字段: mid \t uid \t content \t media_id \t customer_info(pid列表) \t dt
#
# 用法：
#   bash sql/query_image.sh                        # 使用默认日期
#   bash sql/query_image.sh 20260701               # 单日
#   bash sql/query_image.sh 20260701 20260731      # 日期范围
#
...
echo "过滤条件:     market_industry_name='汽车', bid_type=4, media_type=1(图片)"
...
#   2. 关联博文内容表，取出文字内容
#   3. 关联媒体表（cust_uid = customer_id），只保留 media_type=1（图片）
#   4. customer_info 字段为 JSON 数组字符串，如 ["pid1","pid2"]
#      下游 Python 脚本负责解析 pid 并构造图片 URL
#
# 注意：dm表与media表通过 cust_uid=customer_id 关联，粒度为广告主，
#       同一广告主可能有多条图片素材，结果会有多行（每行一张图片）
MAIN_SQL="
SELECT DISTINCT
    ad.mid,
    ad.cust_uid                AS uid,
...
echo ""
echo "--- 前 5 条数据预览 (mid / uid / media_id / customer_info前60字 / dt) ---"
hdfs dfs -cat "${HDFS_TARGET_PATH}/part-*" 2>/dev/null \
    | head -5 \
    | awk -F'\t' '{printf "mid=%-20s uid=%-12s media_id=%-22s dt=%s pids=%.60s\n", $1, $2, $4, $6, $5}'
echo "========================================================"
