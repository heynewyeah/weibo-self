#!/usr/bin/env bash
# =============================================================================
# 安装/更新“每周四 10:00（北京时间）发送钉钉项目周报”的 cron 任务。
#
# 前置条件：
...
#   - 使用 TZ=Asia/Shanghai，固定为每周四 10:00；
...
CRON_LINE="0 10 * * 4 cd '$PROJECT_DIR' && DWS_RUNNER='$DWS_RUNNER' '$PYTHON_BIN' scripts/send_weekly_report.py >> logs/weekly_report_cron.log 2>&1 $MARKER"
...
echo "已安装/更新 cron：每周四 10:00（Asia/Shanghai）发送钉钉周报。"
