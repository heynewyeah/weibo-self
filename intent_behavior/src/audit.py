...
class RunAudit:
    ...
    def record_result(self, result, record=None) -> None:
        ...
        payload = {
            ...
            "brand_terms": getattr(result, "brand_terms", []),
            "topic_terms": getattr(result, "topic_terms", []),
            "is_forward": result.is_forward,
            "forward_mid": result.forward_mid,
            "forward_status": result.forward_status,
            "layer": result.layer,
            "level": getattr(result, "level_code", None),
            "writeback_state": getattr(result, "writeback_state", "not_requested"),
            "writeback_response": getattr(result, "writeback_response", {}),
