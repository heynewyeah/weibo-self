...
视频博文 mid 反解 + 分类测试
============================
功能：通过 mid 调用微博内部反解接口获取博文真实内容（正文 + 视频 fid/cover），
      然后使用 Qwen 模型进行视频多模态分类。

为什么需要：
- MySQL 分表里的 mid_fids 字段可能和实际请求 API 所需不匹配。
- 通过 mid 反解可以拿到最新、最准确的视频 fid 和封面图。

运行方式：
  # 单条视频博文测试（默认 cover 模式）
  python3 tests/08_mid_resolver/test_resolve_and_classify_video.py \
      --mid 4876329553760906 --uid 2608812381

  # frame 模式（下载视频 + OpenCV 抽帧）
  python3 tests/08_mid_resolver/test_resolve_and_classify_video.py \
      --mid 4876329553760906 --uid 2608812381 --video-mode frame

  # 指定配置文件
  python3 tests/08_mid_resolver/test_resolve_and_classify_video.py \
      --mid 4876329553760906 --config config/config.yaml

  # 只反解，不做模型分类（用于验证反解结果）
  python3 tests/08_mid_resolver/test_resolve_and_classify_video.py \
      --mid 4876329553760906 --resolve-only

输出路径：
  tests/08_mid_resolver/output/resolve_video_<mid>_<timestamp>.json
...
def main():
    ...
    logger = setup_logger("test_resolve_video", log_dir=os.path.join(PROJECT_DIR, "logs"))

    logger.info("=" * 60)
    logger.info("视频博文 mid 反解 + 分类测试")
    ...
    logger.info(f"  视频 fid: {resolved.video_fid or '无'}")
    logger.info(f"  视频封面: {resolved.video_cover_url or '无'}")
    logger.info(f"  视频 URL: {resolved.video_url or '无'}")

    if args.resolve_only:
        logger.info("--resolve-only 模式，跳过模型分类")
        output_path = os.path.join(OUTPUT_DIR, f"resolve_video_{args.mid}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    ...
    if not resolved.has_video():
        logger.warning("反解结果中没有视频 fid，无法做视频分类，尝试降级为文本分类")
